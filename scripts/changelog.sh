...
if [ -n "$in_place_file" ]; then
  ...
  tmpscrubbed="${in_place_file}.scrubbed.$$"
  if [ -f "$in_place_file" ]; then
    # When generating a versioned release, remove any existing [Unreleased] section
    # so we never end up with two changelog entries for the same work.
    if [ "$release_version" != "Unreleased" ]; then
      awk '
        /^## \[Unreleased\]/ { skip=1; next }
        skip && /^## \[/     { skip=0 }
        !skip                { print }
      ' "$in_place_file" >"$tmpscrubbed"
    else
      cp "$in_place_file" "$tmpscrubbed"
    fi
    if head -n1 "$tmpscrubbed" | grep -q '^# Changelog'; then
      { head -n1 "$tmpscrubbed"; echo; cat "$section"; echo; tail -n +2 "$tmpscrubbed"; } >"$tmpout"
    else
      { echo '# Changelog'; echo; cat "$section"; echo; cat "$tmpscrubbed"; } >"$tmpout"
    fi
    rm -f "$tmpscrubbed"
  else
