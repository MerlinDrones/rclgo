...
type ObjectHypothesis struct {
	ClassId string  `yaml:"class_id"` // The unique ID of the object class. To get additional information aboutthis ID, such as its human-readable class name, listeners should perform alookup in a metadata database. See vision_msgs/VisionInfo.msg for more detail.
	Score   float64 `yaml:"score"`    // The probability or confidence value of the detected object. By convention,this value should lie in the range [0-1].
}
