...
type VisionClass struct {
	ClassId   uint16 `yaml:"class_id"`   // The int value that identifies the class.Elements identified with 65535, the maximum uint16 value are assumedto belong to the "UNLABELED" class. For vision pipelines using lessthan 255 classes the "UNLABELED" is the maximum value in the uint8range.
	ClassName string `yaml:"class_name"` // The name of the class represented by the class_id
}
