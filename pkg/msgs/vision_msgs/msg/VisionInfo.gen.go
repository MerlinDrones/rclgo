...
type VisionInfo struct {
	Header           std_msgs_msg.Header `yaml:"header"`            // Used for sequencing
	Method           string              `yaml:"method"`            // Name of the vision pipeline. This should be a value that is meaningful to anoutside user.
	DatabaseLocation string              `yaml:"database_location"` // Location where the metadata database is stored. The recommended location isas an XML string on the ROS parameter server, but the exact implementationand information is left up to the user.The database should store information attached to class ids. Eachclass id should map to an atomic, visually recognizable element. Thisdefinition is intentionally vague to allow extreme flexibility. Theelements could be classes in a pixel segmentation algorithm, object classesin a detector, different people's faces in a face detection algorithm, etc.Vision pipelines report results in terms of numeric IDs, which map intothis  database.The information stored in this database is, again, left up to the user. Thedatabase could be as simple as a map from ID to class name, or it couldinclude information such as object meshes or colors to use forvisualization.
	DatabaseVersion  int32               `yaml:"database_version"`  // Metadata database version. This counter is incrementedeach time the pipeline begins using a new version of the database (usefulin the case of online training or user modifications).The counter value can be monitored by listeners to ensure that the pipelineand the listener are using the same metadata.
}
