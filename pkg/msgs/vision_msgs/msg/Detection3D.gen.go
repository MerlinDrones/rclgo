...
type Detection3D struct {
	...
	Results []ObjectHypothesisWithPose `yaml:"results"` // Class probabilities. Does not have to include hypotheses for all possibleobject ids, the scores for any ids not listed are assumed to be 0.
	Bbox    BoundingBox3D              `yaml:"bbox"`    // 3D bounding box surrounding the object.
	Id      string                     `yaml:"id"`      // ID used for consistency across multiple detection messages. Detectionsof the same object in different detection messages should have the same id.This field may be empty.
}
