...
func TestOnGetParameters_UnknownAndKnown(t *testing.T) {
	...
	req := rclif_srv.NewGetParameters_Request()
	req.Names = []string{"camera.fps", "missing"}

	s := &captureSender{}
	m.onGetParameters((*rclgo.ServiceInfo)(nil), req, s)

	resp := s.last.(*rclif_srv.GetParameters_Response)
...
func TestOnSetParameters_SetsStoreAndReturnsResult(t *testing.T) {
	...
	req := rclif_srv.NewSetParameters_Request()
	req.Parameters = []rclif_msg.Parameter{
		{
			Name: "gain",
	...
	resp := s.last.(*rclif_srv.SetParameters_Response)
...
func TestOnSetParameters_PerItemResultsLength(t *testing.T) {
	...
	req := rclif_srv.NewSetParameters_Request()
	req.Parameters = []rclif_msg.Parameter{
		{Name: "a", Value: rclif_msg.ParameterValue{Type: KindInt64, IntegerValue: 10}},
		{Name: "b", Value: rclif_msg.ParameterValue{Type: KindInt64, IntegerValue: 20}},
	}
...
func TestOnListParameters_ReturnsAllNames(t *testing.T) {
	...
	req := rclif_srv.NewListParameters_Request()
	s := &captureSender{}
	m.onListParameters(nil, req, s)

	resp := s.last.(*rclif_srv.ListParameters_Response)
	got := map[string]bool{}
