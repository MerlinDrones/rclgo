...
func init() {
	...
	typemap.RegisterMessage("px4_msgs/msg/OrbitStatus", OrbitStatusTypeSupport)
}

const (
	OrbitStatus_ORBIT_YAW_BEHAVIOUR_HOLD_FRONT_TO_CIRCLE_CENTER  uint8 = 0 // ORBIT_YAW_BEHAVIOUR
	OrbitStatus_ORBIT_YAW_BEHAVIOUR_HOLD_INITIAL_HEADING         uint8 = 1
	OrbitStatus_ORBIT_YAW_BEHAVIOUR_UNCONTROLLED                 uint8 = 2
	OrbitStatus_ORBIT_YAW_BEHAVIOUR_HOLD_FRONT_TANGENT_TO_CIRCLE uint8 = 3
	OrbitStatus_ORBIT_YAW_BEHAVIOUR_RC_CONTROLLED                uint8 = 4
	OrbitStatus_ORBIT_YAW_BEHAVIOUR_UNCHANGED                    uint8 = 5
)
...
func (t *OrbitStatus) GetTypeSupport() types.MessageTypeSupport {
	return OrbitStatusTypeSupport
}

// OrbitStatusPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type OrbitStatusPublisher struct {
	*rclgo.Publisher
}

// NewOrbitStatusPublisher creates and returns a new publisher for the
// OrbitStatus
func NewOrbitStatusPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*OrbitStatusPublisher, error) {
	pub, err := node.NewPublisher(topic_name, OrbitStatusTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &OrbitStatusPublisher{pub}, nil
}

func (p *OrbitStatusPublisher) Publish(msg *OrbitStatus) error {
	return p.Publisher.Publish(msg)
}

// OrbitStatusSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type OrbitStatusSubscription struct {
	*rclgo.Subscription
}

// OrbitStatusSubscriptionCallback type is used to provide a subscription
// handler function for a OrbitStatusSubscription.
type OrbitStatusSubscriptionCallback func(msg *OrbitStatus, info *rclgo.MessageInfo, err error)

// NewOrbitStatusSubscription creates and returns a new subscription for the
// OrbitStatus
func NewOrbitStatusSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback OrbitStatusSubscriptionCallback) (*OrbitStatusSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg OrbitStatus
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, OrbitStatusTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &OrbitStatusSubscription{sub}, nil
}

func (s *OrbitStatusSubscription) TakeMessage(out *OrbitStatus) (*rclgo.MessageInfo, error) {
...
func OrbitStatus__Sequence_to_Go(goSlice *[]OrbitStatus, cSlice COrbitStatus__Sequence) {
	...
	*goSlice = make([]OrbitStatus, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		OrbitStatusTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func OrbitStatus__Sequence_to_C(cSlice *COrbitStatus__Sequence, goSlice []OrbitStatus) {
	...
	cSlice.data = (*C.px4_msgs__msg__OrbitStatus)(C.malloc(C.sizeof_struct_px4_msgs__msg__OrbitStatus * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		OrbitStatusTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
