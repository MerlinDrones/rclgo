...
func init() {
	...
	typemap.RegisterMessage("px4_msgs/msg/NavigatorMissionItem", NavigatorMissionItemTypeSupport)
}
...
func (t *NavigatorMissionItem) GetTypeSupport() types.MessageTypeSupport {
	return NavigatorMissionItemTypeSupport
}

// NavigatorMissionItemPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type NavigatorMissionItemPublisher struct {
	*rclgo.Publisher
}

// NewNavigatorMissionItemPublisher creates and returns a new publisher for the
// NavigatorMissionItem
func NewNavigatorMissionItemPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*NavigatorMissionItemPublisher, error) {
	pub, err := node.NewPublisher(topic_name, NavigatorMissionItemTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &NavigatorMissionItemPublisher{pub}, nil
}

func (p *NavigatorMissionItemPublisher) Publish(msg *NavigatorMissionItem) error {
	return p.Publisher.Publish(msg)
}

// NavigatorMissionItemSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type NavigatorMissionItemSubscription struct {
	*rclgo.Subscription
}

// NavigatorMissionItemSubscriptionCallback type is used to provide a subscription
// handler function for a NavigatorMissionItemSubscription.
type NavigatorMissionItemSubscriptionCallback func(msg *NavigatorMissionItem, info *rclgo.MessageInfo, err error)

// NewNavigatorMissionItemSubscription creates and returns a new subscription for the
// NavigatorMissionItem
func NewNavigatorMissionItemSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback NavigatorMissionItemSubscriptionCallback) (*NavigatorMissionItemSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg NavigatorMissionItem
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, NavigatorMissionItemTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &NavigatorMissionItemSubscription{sub}, nil
}

func (s *NavigatorMissionItemSubscription) TakeMessage(out *NavigatorMissionItem) (*rclgo.MessageInfo, error) {
...
func NavigatorMissionItem__Sequence_to_Go(goSlice *[]NavigatorMissionItem, cSlice CNavigatorMissionItem__Sequence) {
	...
	*goSlice = make([]NavigatorMissionItem, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		NavigatorMissionItemTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func NavigatorMissionItem__Sequence_to_C(cSlice *CNavigatorMissionItem__Sequence, goSlice []NavigatorMissionItem) {
	...
	cSlice.data = (*C.px4_msgs__msg__NavigatorMissionItem)(C.malloc(C.sizeof_struct_px4_msgs__msg__NavigatorMissionItem * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		NavigatorMissionItemTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
