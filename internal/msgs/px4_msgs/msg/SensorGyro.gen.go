...
func init() {
	...
	typemap.RegisterMessage("px4_msgs/msg/SensorGyro", SensorGyroTypeSupport)
}
...
func (t *SensorGyro) GetTypeSupport() types.MessageTypeSupport {
	return SensorGyroTypeSupport
}

// SensorGyroPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type SensorGyroPublisher struct {
	*rclgo.Publisher
}

// NewSensorGyroPublisher creates and returns a new publisher for the
// SensorGyro
func NewSensorGyroPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*SensorGyroPublisher, error) {
	pub, err := node.NewPublisher(topic_name, SensorGyroTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &SensorGyroPublisher{pub}, nil
}

func (p *SensorGyroPublisher) Publish(msg *SensorGyro) error {
	return p.Publisher.Publish(msg)
}

// SensorGyroSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type SensorGyroSubscription struct {
	*rclgo.Subscription
}

// SensorGyroSubscriptionCallback type is used to provide a subscription
// handler function for a SensorGyroSubscription.
type SensorGyroSubscriptionCallback func(msg *SensorGyro, info *rclgo.MessageInfo, err error)

// NewSensorGyroSubscription creates and returns a new subscription for the
// SensorGyro
func NewSensorGyroSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback SensorGyroSubscriptionCallback) (*SensorGyroSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg SensorGyro
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, SensorGyroTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &SensorGyroSubscription{sub}, nil
}

func (s *SensorGyroSubscription) TakeMessage(out *SensorGyro) (*rclgo.MessageInfo, error) {
...
func SensorGyro__Sequence_to_Go(goSlice *[]SensorGyro, cSlice CSensorGyro__Sequence) {
	...
	*goSlice = make([]SensorGyro, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		SensorGyroTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func SensorGyro__Sequence_to_C(cSlice *CSensorGyro__Sequence, goSlice []SensorGyro) {
	...
	cSlice.data = (*C.px4_msgs__msg__SensorGyro)(C.malloc(C.sizeof_struct_px4_msgs__msg__SensorGyro * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		SensorGyroTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
