...
func init() {
	...
	typemap.RegisterMessage("px4_msgs/msg/GeneratorStatus", GeneratorStatusTypeSupport)
}
...
func (t *GeneratorStatus) GetTypeSupport() types.MessageTypeSupport {
	return GeneratorStatusTypeSupport
}

// GeneratorStatusPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type GeneratorStatusPublisher struct {
	*rclgo.Publisher
}

// NewGeneratorStatusPublisher creates and returns a new publisher for the
// GeneratorStatus
func NewGeneratorStatusPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*GeneratorStatusPublisher, error) {
	pub, err := node.NewPublisher(topic_name, GeneratorStatusTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &GeneratorStatusPublisher{pub}, nil
}

func (p *GeneratorStatusPublisher) Publish(msg *GeneratorStatus) error {
	return p.Publisher.Publish(msg)
}

// GeneratorStatusSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type GeneratorStatusSubscription struct {
	*rclgo.Subscription
}

// GeneratorStatusSubscriptionCallback type is used to provide a subscription
// handler function for a GeneratorStatusSubscription.
type GeneratorStatusSubscriptionCallback func(msg *GeneratorStatus, info *rclgo.MessageInfo, err error)

// NewGeneratorStatusSubscription creates and returns a new subscription for the
// GeneratorStatus
func NewGeneratorStatusSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback GeneratorStatusSubscriptionCallback) (*GeneratorStatusSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg GeneratorStatus
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, GeneratorStatusTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &GeneratorStatusSubscription{sub}, nil
}

func (s *GeneratorStatusSubscription) TakeMessage(out *GeneratorStatus) (*rclgo.MessageInfo, error) {
...
func GeneratorStatus__Sequence_to_Go(goSlice *[]GeneratorStatus, cSlice CGeneratorStatus__Sequence) {
	...
	*goSlice = make([]GeneratorStatus, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		GeneratorStatusTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func GeneratorStatus__Sequence_to_C(cSlice *CGeneratorStatus__Sequence, goSlice []GeneratorStatus) {
	...
	cSlice.data = (*C.px4_msgs__msg__GeneratorStatus)(C.malloc(C.sizeof_struct_px4_msgs__msg__GeneratorStatus * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		GeneratorStatusTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
