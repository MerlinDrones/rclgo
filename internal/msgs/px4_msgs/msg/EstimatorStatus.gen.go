...
func init() {
	...
	typemap.RegisterMessage("px4_msgs/msg/EstimatorStatus", EstimatorStatusTypeSupport)
}
...
const (
	...
	EstimatorStatus_GPS_CHECK_FAIL_MIN_SAT_COUNT    uint8 = 1  // 1 : minimum required sat count fail
	EstimatorStatus_GPS_CHECK_FAIL_MAX_PDOP         uint8 = 2  // 2 : maximum allowed PDOP fail
	EstimatorStatus_GPS_CHECK_FAIL_MAX_HORZ_ERR     uint8 = 3  // 3 : maximum allowed horizontal position error fail
	EstimatorStatus_GPS_CHECK_FAIL_MAX_VERT_ERR     uint8 = 4  // 4 : maximum allowed vertical position error fail
	EstimatorStatus_GPS_CHECK_FAIL_MAX_SPD_ERR      uint8 = 5  // 5 : maximum allowed speed error fail
	EstimatorStatus_GPS_CHECK_FAIL_MAX_HORZ_DRIFT   uint8 = 6  // 6 : maximum allowed horizontal position drift fail - requires stationary vehicle
	EstimatorStatus_GPS_CHECK_FAIL_MAX_VERT_DRIFT   uint8 = 7  // 7 : maximum allowed vertical position drift fail - requires stationary vehicle
	EstimatorStatus_GPS_CHECK_FAIL_MAX_HORZ_SPD_ERR uint8 = 8  // 8 : maximum allowed horizontal speed fail - requires stationary vehicle
	EstimatorStatus_GPS_CHECK_FAIL_MAX_VERT_SPD_ERR uint8 = 9  // 9 : maximum allowed vertical velocity discrepancy fail
	EstimatorStatus_GPS_CHECK_FAIL_SPOOFED          uint8 = 10 // 10 : GPS signal is spoofed
	...
	EstimatorStatus_CS_MAG_FAULT                    uint8 = 18 // 18 - true when the magnetometer has been declared faulty and is no longer being used
	...
	EstimatorStatus_CS_RNG_FAULT                    uint8 = 28 // 28 - true when the range finder has been declared faulty and is no longer being used
)
...
type EstimatorStatus struct {
	...
	ControlModeFlags            uint64     `yaml:"control_mode_flags"`    // Bitmask to indicate EKF logic state
	...
	HdgTestRatio                float32    `yaml:"hdg_test_ratio"`        // low-pass filtered ratio of the largest heading innovation component to the innovation test limit
	VelTestRatio                float32    `yaml:"vel_test_ratio"`        // low-pass filtered ratio of the largest velocity innovation component to the innovation test limit
	PosTestRatio                float32    `yaml:"pos_test_ratio"`        // low-pass filtered ratio of the largest horizontal position innovation component to the innovation test limit
	HgtTestRatio                float32    `yaml:"hgt_test_ratio"`        // low-pass filtered ratio of the vertical position innovation to the innovation test limit
	TasTestRatio                float32    `yaml:"tas_test_ratio"`        // low-pass filtered ratio of the true airspeed innovation to the innovation test limit
	HaglTestRatio               float32    `yaml:"hagl_test_ratio"`       // low-pass filtered ratio of the height above ground innovation to the innovation test limit
	BetaTestRatio               float32    `yaml:"beta_test_ratio"`       // low-pass filtered ratio of the synthetic sideslip innovation to the innovation test limit
	...
	PreFltFailInnovPosHoriz     bool       `yaml:"pre_flt_fail_innov_pos_horiz"`
	...
	TimeoutFlags                uint8      `yaml:"timeout_flags"` // Bitmask to indicate timeout flags (vel, pos, hgt)
	MagInclinationDeg           float32    `yaml:"mag_inclination_deg"`
	MagInclinationRefDeg        float32    `yaml:"mag_inclination_ref_deg"`
	MagStrengthGs               float32    `yaml:"mag_strength_gs"`
	MagStrengthRefGs            float32    `yaml:"mag_strength_ref_gs"`
}
...
func (t *EstimatorStatus) Clone() *EstimatorStatus {
	...
	c.PreFltFailInnovPosHoriz = t.PreFltFailInnovPosHoriz
	...
	c.MagInclinationDeg = t.MagInclinationDeg
	c.MagInclinationRefDeg = t.MagInclinationRefDeg
	c.MagStrengthGs = t.MagStrengthGs
	c.MagStrengthRefGs = t.MagStrengthRefGs
	return c
}
...
func (t *EstimatorStatus) SetDefaults() {
	...
	t.PreFltFailInnovPosHoriz = false
	...
	t.HealthFlags = 0
	t.TimeoutFlags = 0
	t.MagInclinationDeg = 0
	t.MagInclinationRefDeg = 0
	t.MagStrengthGs = 0
	t.MagStrengthRefGs = 0
}
...
func (t _EstimatorStatusTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	mem.control_mode_flags = C.uint64_t(m.ControlModeFlags)
	...
	mem.hdg_test_ratio = C.float(m.HdgTestRatio)
	...
	mem.pre_flt_fail_innov_pos_horiz = C.bool(m.PreFltFailInnovPosHoriz)
	...
	mem.mag_inclination_deg = C.float(m.MagInclinationDeg)
	mem.mag_inclination_ref_deg = C.float(m.MagInclinationRefDeg)
	mem.mag_strength_gs = C.float(m.MagStrengthGs)
	mem.mag_strength_ref_gs = C.float(m.MagStrengthRefGs)
}
...
func (t _EstimatorStatusTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	m.ControlModeFlags = uint64(mem.control_mode_flags)
	...
	m.HdgTestRatio = float32(mem.hdg_test_ratio)
	...
	m.PreFltFailInnovPosHoriz = bool(mem.pre_flt_fail_innov_pos_horiz)
	...
	m.MagInclinationDeg = float32(mem.mag_inclination_deg)
	m.MagInclinationRefDeg = float32(mem.mag_inclination_ref_deg)
	m.MagStrengthGs = float32(mem.mag_strength_gs)
	m.MagStrengthRefGs = float32(mem.mag_strength_ref_gs)
}
...
func EstimatorStatus__Sequence_to_C(cSlice *CEstimatorStatus__Sequence, goSlice []EstimatorStatus) {
	...
	cSlice.data = (*C.px4_msgs__msg__EstimatorStatus)(C.malloc(C.sizeof_struct_px4_msgs__msg__EstimatorStatus * C.size_t(len(goSlice))))
