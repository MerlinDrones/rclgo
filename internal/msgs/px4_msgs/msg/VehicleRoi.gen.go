...
func init() {
	...
	typemap.RegisterMessage("px4_msgs/msg/VehicleRoi", VehicleRoiTypeSupport)
}
...
type VehicleRoi struct {
	...
	PitchOffset float32 `yaml:"pitch_offset"` // angle offset in rad
	YawOffset   float32 `yaml:"yaw_offset"`   // angle offset in rad
}
...
func (t *VehicleRoi) GetTypeSupport() types.MessageTypeSupport {
	return VehicleRoiTypeSupport
}

// VehicleRoiPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type VehicleRoiPublisher struct {
	*rclgo.Publisher
}

// NewVehicleRoiPublisher creates and returns a new publisher for the
// VehicleRoi
func NewVehicleRoiPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*VehicleRoiPublisher, error) {
	pub, err := node.NewPublisher(topic_name, VehicleRoiTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &VehicleRoiPublisher{pub}, nil
}

func (p *VehicleRoiPublisher) Publish(msg *VehicleRoi) error {
	return p.Publisher.Publish(msg)
}

// VehicleRoiSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type VehicleRoiSubscription struct {
	*rclgo.Subscription
}

// VehicleRoiSubscriptionCallback type is used to provide a subscription
// handler function for a VehicleRoiSubscription.
type VehicleRoiSubscriptionCallback func(msg *VehicleRoi, info *rclgo.MessageInfo, err error)

// NewVehicleRoiSubscription creates and returns a new subscription for the
// VehicleRoi
func NewVehicleRoiSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback VehicleRoiSubscriptionCallback) (*VehicleRoiSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg VehicleRoi
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, VehicleRoiTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &VehicleRoiSubscription{sub}, nil
}

func (s *VehicleRoiSubscription) TakeMessage(out *VehicleRoi) (*rclgo.MessageInfo, error) {
...
func VehicleRoi__Sequence_to_Go(goSlice *[]VehicleRoi, cSlice CVehicleRoi__Sequence) {
	...
	*goSlice = make([]VehicleRoi, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		VehicleRoiTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func VehicleRoi__Sequence_to_C(cSlice *CVehicleRoi__Sequence, goSlice []VehicleRoi) {
	...
	cSlice.data = (*C.px4_msgs__msg__VehicleRoi)(C.malloc(C.sizeof_struct_px4_msgs__msg__VehicleRoi * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		VehicleRoiTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
