...
func init() {
	...
	typemap.RegisterMessage("px4_msgs/msg/HoverThrustEstimate", HoverThrustEstimateTypeSupport)
}
...
func (t *HoverThrustEstimate) GetTypeSupport() types.MessageTypeSupport {
	return HoverThrustEstimateTypeSupport
}

// HoverThrustEstimatePublisher wraps rclgo.Publisher to provide type safe helper
// functions
type HoverThrustEstimatePublisher struct {
	*rclgo.Publisher
}

// NewHoverThrustEstimatePublisher creates and returns a new publisher for the
// HoverThrustEstimate
func NewHoverThrustEstimatePublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*HoverThrustEstimatePublisher, error) {
	pub, err := node.NewPublisher(topic_name, HoverThrustEstimateTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &HoverThrustEstimatePublisher{pub}, nil
}

func (p *HoverThrustEstimatePublisher) Publish(msg *HoverThrustEstimate) error {
	return p.Publisher.Publish(msg)
}

// HoverThrustEstimateSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type HoverThrustEstimateSubscription struct {
	*rclgo.Subscription
}

// HoverThrustEstimateSubscriptionCallback type is used to provide a subscription
// handler function for a HoverThrustEstimateSubscription.
type HoverThrustEstimateSubscriptionCallback func(msg *HoverThrustEstimate, info *rclgo.MessageInfo, err error)

// NewHoverThrustEstimateSubscription creates and returns a new subscription for the
// HoverThrustEstimate
func NewHoverThrustEstimateSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback HoverThrustEstimateSubscriptionCallback) (*HoverThrustEstimateSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg HoverThrustEstimate
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, HoverThrustEstimateTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &HoverThrustEstimateSubscription{sub}, nil
}

func (s *HoverThrustEstimateSubscription) TakeMessage(out *HoverThrustEstimate) (*rclgo.MessageInfo, error) {
...
func HoverThrustEstimate__Sequence_to_Go(goSlice *[]HoverThrustEstimate, cSlice CHoverThrustEstimate__Sequence) {
	...
	*goSlice = make([]HoverThrustEstimate, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		HoverThrustEstimateTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func HoverThrustEstimate__Sequence_to_C(cSlice *CHoverThrustEstimate__Sequence, goSlice []HoverThrustEstimate) {
	...
	cSlice.data = (*C.px4_msgs__msg__HoverThrustEstimate)(C.malloc(C.sizeof_struct_px4_msgs__msg__HoverThrustEstimate * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		HoverThrustEstimateTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
