...
func init() {
	...
	typemap.RegisterMessage("px4_msgs/msg/VehicleCommand", VehicleCommandTypeSupport)
}
...
const (
	...
	VehicleCommand_VEHICLE_CMD_DO_FIGUREEIGHT                       uint16 = 35   // Start flying on the outline of a figure eight defined by the parameters. |Major Radius [m] |Minor Radius [m] |Velocity [m/s] |Orientation |Latitude/X |Longitude/Y |Altitude/Z |
	...
	VehicleCommand_VEHICLE_CMD_DO_REPEAT_RELAY                      uint16 = 182  // Cycle a relay on and off for a desired number of cycles with a desired period. |Relay number| Cycle count| Cycle time (seconds, decimal)| Empty| Empty| Empty| Empty|
	...
	VehicleCommand_VEHICLE_CMD_DO_CHANGE_ALTITUDE                   uint16 = 186  // Set the vehicle to Loiter mode and change the altitude to specified value |Altitude| Frame of new altitude | Empty| Empty| Empty| Empty| Empty|
	VehicleCommand_VEHICLE_CMD_DO_SET_ACTUATOR                      uint16 = 187  // Sets actuators (e.g. servos) to a desired value. |Actuator 1| Actuator 2| Actuator 3| Actuator 4| Actuator 5| Actuator 6| Index|
	...
	VehicleCommand_VEHICLE_CMD_DO_GO_AROUND                         uint16 = 191  // Mission command to safely abort an autonomous landing. |Altitude (meters)| Empty| Empty| Empty| Empty| Empty| Empty|
	VehicleCommand_VEHICLE_CMD_DO_REPOSITION                        uint16 = 192  // Reposition to specific WGS84 GPS position. |Ground speed [m/s] |Bitmask |Loiter radius [m] for planes |Yaw	[deg] |Latitude	|Longitude |Altitude |
	...
	VehicleCommand_VEHICLE_CMD_DO_GRIPPER                           uint16 = 211 // Command to operate a gripper
	...
	VehicleCommand_VEHICLE_CMD_DO_SET_STANDARD_MODE                 uint16 = 262 // Enable the specified standard MAVLink mode |MAV_STANDARD_MODE|
	...
	VehicleCommand_VEHICLE_CMD_ACTUATOR_TEST                        uint16 = 310 // Actuator testing command|value [-1,1]|timeout [s]|Empty|Empty|output function|
	VehicleCommand_VEHICLE_CMD_CONFIGURE_ACTUATOR                   uint16 = 311 // Actuator configuration command|configuration|Empty|Empty|Empty|output function|
	VehicleCommand_VEHICLE_CMD_COMPONENT_ARM_DISARM                 uint16 = 400 // Arms / Disarms a component |1 to arm, 0 to disarm
	VehicleCommand_VEHICLE_CMD_RUN_PREARM_CHECKS                    uint16 = 401 // Instructs a target system to run pre-arm checks.
	...
	VehicleCommand_VEHICLE_CMD_REQUEST_CAMERA_INFORMATION           uint16 = 521 // Request camera information (CAMERA_INFORMATION). |0: No action 1: Request camera capabilities| Reserved (all remaining params)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)| Reserved (default:0)|
	...
	VehicleCommand_VEHICLE_CMD_IMAGE_START_CAPTURE                  uint16 = 2000  // Start image capture sequence.
	VehicleCommand_VEHICLE_CMD_DO_TRIGGER_CONTROL                   uint16 = 2003  // Enable or disable on-board camera triggering system
	VehicleCommand_VEHICLE_CMD_VIDEO_START_CAPTURE                  uint16 = 2500  // Start a video capture.
	VehicleCommand_VEHICLE_CMD_VIDEO_STOP_CAPTURE                   uint16 = 2501  // Stop the current video capture.
	...
	VehicleCommand_VEHICLE_CMD_DO_WINCH                             uint16 = 42600 // Command to operate winch.
	VehicleCommand_VEHICLE_CMD_EXTERNAL_POSITION_ESTIMATE           uint16 = 43003 // external reset of estimator global position when deadreckoning
	VehicleCommand_VEHICLE_CMD_EXTERNAL_WIND_ESTIMATE               uint16 = 43004
	...
	VehicleCommand_VEHICLE_CMD_SET_GPS_GLOBAL_ORIGIN                uint32 = 100000 // Sets the GPS coordinates of the vehicle local origin (0,0,0) position. |Empty|Empty|Empty|Empty|Latitude|Longitude|Altitude|
	VehicleCommand_VEHICLE_CMD_SET_NAV_STATE                        uint32 = 100001 // Change mode by specifying nav_state directly. |nav_state|Empty|Empty|Empty|Empty|Empty|Empty|
	...
	VehicleCommand_SPEED_TYPE_AIRSPEED                              uint8  = 0 // used as param1 in DO_CHANGE_SPEED command
	VehicleCommand_SPEED_TYPE_GROUNDSPEED                           uint8  = 1
	VehicleCommand_SPEED_TYPE_CLIMB_SPEED                           uint8  = 2
	VehicleCommand_SPEED_TYPE_DESCEND_SPEED                         uint8  = 3
	VehicleCommand_ORBIT_YAW_BEHAVIOUR_HOLD_FRONT_TO_CIRCLE_CENTER  uint8  = 0 // used as param3 in CMD_DO_ORBIT
	VehicleCommand_ORBIT_YAW_BEHAVIOUR_HOLD_INITIAL_HEADING         uint8  = 1
	VehicleCommand_ORBIT_YAW_BEHAVIOUR_UNCONTROLLED                 uint8  = 2
	VehicleCommand_ORBIT_YAW_BEHAVIOUR_HOLD_FRONT_TANGENT_TO_CIRCLE uint8  = 3
	VehicleCommand_ORBIT_YAW_BEHAVIOUR_RC_CONTROLLED                uint8  = 4
	VehicleCommand_ORBIT_YAW_BEHAVIOUR_UNCHANGED                    uint8  = 5
	VehicleCommand_ARMING_ACTION_DISARM                             int8   = 0 // used as param1 in ARM_DISARM command
	VehicleCommand_ARMING_ACTION_ARM                                int8   = 1
	VehicleCommand_GRIPPER_ACTION_RELEASE                           uint8  = 0 // param2 in VEHICLE_CMD_DO_GRIPPER
	VehicleCommand_GRIPPER_ACTION_GRAB                              uint8  = 1
	VehicleCommand_ORB_QUEUE_LENGTH                                 uint8  = 8
	VehicleCommand_COMPONENT_MODE_EXECUTOR_START                    uint16 = 1000
)
...
type VehicleCommand struct {
	...
	SourceComponent uint16  `yaml:"source_component"` // Component / mode executor sending the command
...
func (t *VehicleCommand) GetTypeSupport() types.MessageTypeSupport {
	return VehicleCommandTypeSupport
}

// VehicleCommandPublisher wraps rclgo.Publisher to provide type safe helper
// functions
type VehicleCommandPublisher struct {
	*rclgo.Publisher
}

// NewVehicleCommandPublisher creates and returns a new publisher for the
// VehicleCommand
func NewVehicleCommandPublisher(node *rclgo.Node, topic_name string, options *rclgo.PublisherOptions) (*VehicleCommandPublisher, error) {
	pub, err := node.NewPublisher(topic_name, VehicleCommandTypeSupport, options)
	if err != nil {
		return nil, err
	}
	return &VehicleCommandPublisher{pub}, nil
}

func (p *VehicleCommandPublisher) Publish(msg *VehicleCommand) error {
	return p.Publisher.Publish(msg)
}

// VehicleCommandSubscription wraps rclgo.Subscription to provide type safe helper
// functions
type VehicleCommandSubscription struct {
	*rclgo.Subscription
}

// VehicleCommandSubscriptionCallback type is used to provide a subscription
// handler function for a VehicleCommandSubscription.
type VehicleCommandSubscriptionCallback func(msg *VehicleCommand, info *rclgo.MessageInfo, err error)

// NewVehicleCommandSubscription creates and returns a new subscription for the
// VehicleCommand
func NewVehicleCommandSubscription(node *rclgo.Node, topic_name string, opts *rclgo.SubscriptionOptions, subscriptionCallback VehicleCommandSubscriptionCallback) (*VehicleCommandSubscription, error) {
	callback := func(s *rclgo.Subscription) {
		var msg VehicleCommand
		info, err := s.TakeMessage(&msg)
		subscriptionCallback(&msg, info, err)
	}
	sub, err := node.NewSubscription(topic_name, VehicleCommandTypeSupport, opts, callback)
	if err != nil {
		return nil, err
	}
	return &VehicleCommandSubscription{sub}, nil
}

func (s *VehicleCommandSubscription) TakeMessage(out *VehicleCommand) (*rclgo.MessageInfo, error) {
...
func (t _VehicleCommandTypeSupport) AsCStruct(dst unsafe.Pointer, msg types.Message) {
	...
	mem.source_component = C.uint16_t(m.SourceComponent)
...
func (t _VehicleCommandTypeSupport) AsGoStruct(msg types.Message, ros2_message_buffer unsafe.Pointer) {
	...
	m.SourceComponent = uint16(mem.source_component)
...
func VehicleCommand__Sequence_to_Go(goSlice *[]VehicleCommand, cSlice CVehicleCommand__Sequence) {
	...
	*goSlice = make([]VehicleCommand, cSlice.size)
	src := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range src {
		VehicleCommandTypeSupport.AsGoStruct(&(*goSlice)[i], unsafe.Pointer(&src[i]))
	}
}
func VehicleCommand__Sequence_to_C(cSlice *CVehicleCommand__Sequence, goSlice []VehicleCommand) {
	...
	cSlice.data = (*C.px4_msgs__msg__VehicleCommand)(C.malloc(C.sizeof_struct_px4_msgs__msg__VehicleCommand * C.size_t(len(goSlice))))
	cSlice.capacity = C.size_t(len(goSlice))
	cSlice.size = cSlice.capacity
	dst := unsafe.Slice(cSlice.data, cSlice.size)
	for i := range goSlice {
		VehicleCommandTypeSupport.AsCStruct(unsafe.Pointer(&dst[i]), &goSlice[i])
	}
}
